...
- `Reflect.preventExtensions()` throws a  if the target is not an object, while `Object.preventExtensions()` always returns non-object targets as-is.
...
If the `target` argument to this method is not an object (a primitive), then it will cause a . With {{jsxref("Object.preventExtensions()")}}, a non-object `target` will be returned as-is without any errors.
